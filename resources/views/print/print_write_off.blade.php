...
<div class="container">
  ...
  <div class="header">
    ...
    <h4>(125272-P)</h4>
    <h2 style="margin:20px 30%;border:1px solid black">STOCK WRITE OFF</h2>
  </div>
    ...
    <div class="second">
      <table style="float:right;font-size:19px">
        <tr>
          <td>Reference No</td>
          <td>:</td>
          <td align="right">{{$wf->seq_no}}</td>
        </tr>
        <tr>
          <td>Issue Date</td>
          <td>:</td>
          <td align="right">{{date("d-M-Y h:i:s A",strtotime($wf->write_off_date))}}</td>
        </tr>
        <tr>
          <td></td>
        </tr>
    ...
    <table class="detail" style="width:100%;margin-top:30px;border-collapse: collapse;">
      ...
      <tbody class="border">
        ...
        <tr>
          ...
          <td style="text-align: right"><strong>Rm {{number_format($wf->total_amount,2)}}</strong></td>
        </tr>
      </tbody>
    </table>
  </div>
