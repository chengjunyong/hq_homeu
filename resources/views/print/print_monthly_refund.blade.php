...
<div style="margin: 0 30px;">
  <div class="header">
    ...
    <h4>(125272-P)</h4>
    <h4 style="margin:20px 30%;border:1px solid black">Monthly Refund Report <br/>({{ date('Y F',strtotime($target_date)) }})</h3>
  </div>
  ...
  <div class="main" style="">
    <table class="detail" style="width:100%;margin-top:30px;border-collapse: collapse;margin-bottom:20px;">
      <thead>
        ...
        <td align="center" style="width: 10%;">Refund No</td>
        <td align="center" style="width: 10%;">Counter Name</td>
        <td align="center" style="width: 10%;">Cashier Name</td>
        <td align="right" style="width: 5%;">Total</td>
        <td align="center" style="width: 10%;">Refund Date</td>
      </thead>
      <tbody>
        @foreach($refund as $index => $result)
          <tr>
            <td align="center">{{$index +1}}</td>
            <td>{{$result->transaction_no}}</td>
            <td align="center">{{$result->cashier_name}}</td>
            <td align="center">{{$result->created_by}}</td>
            <td align="right">{{number_format($result->total,2)}}</td>
            <td align="center">{{date("Y-m-d h:i:s A",strtotime($result->refund_created_at))}}</td>
          </tr>
        @endforeach
      </tbody>
    </table>
