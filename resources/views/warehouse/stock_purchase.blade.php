        ...
        <div class="row scroll" style="overflow-y: scroll;height: 55vh;">
          <div class="col-md-12">
            <table class="table table-bordered">
              ...
              <tfoot>
                <tr>
                  <td colspan=5 style="border:2px solid #e5e5e5">Total Product :</td>
                  <td align="right" style="border:2px solid #e5e5e5" id="total_product">{{$total->product}}</td>
                </tr>
                <tr>
                  <td colspan=5 style="border:2px solid #e5e5e5">Total Quantity :</td>
                  <td align="right" style="border:2px solid #e5e5e5" id="total_quantity">{{$total->quantity}}</td>
                </tr>

                <!-- Auto Tax feature -->
<!--                 <tr>
                  <td colspan=5 style="border:2px solid #e5e5e5">Tax (%):</td>
                  <td style="border:2px solid #e5e5e5;width:20%">
                    <input type="number" name="tax" class="form-control" value="0" step="0.01" min="0" max="100" style="width: 100%;height: 3vh;text-align: right;"/>
                  </td>
                </tr> -->
                <!-- Auto Tax feature -->
                
                <tr>
                  <td colspan=5 style="border:2px solid #e5e5e5">Total Amount :</td>
                  <td align="right" style="border:2px solid #e5e5e5">Rm <label id="total_amount" val="{{$total->amount}}" style="margin-bottom: 0px;">{{number_format($total->amount,2)}}</label></td>
                </tr>
              </tfoot>
            </table>
          </div>
  ...
  $("#check_barcode").click(function(data){
    $.get('{{route('ajaxSearchBar')}}',
    {
      ...
      if(data != false){
        ...
        if(data['measurement'] == 'unit'){
          $("#modal_quantity").attr('min',1);
          $("#modal_quantity").attr('step',1);
        }else{
          $("#modal_quantity").attr('min',0.001);
          $("#modal_quantity").attr('step',0.001);
        }
      }else{
